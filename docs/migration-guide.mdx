...
Extended thinking is now accessed via `ThinkingPart` in message parts instead of
a dedicated `thinking` property:
...
- No need to import `json_schema` package; `Schema` is re-exported from
  `dartantic_ai`
