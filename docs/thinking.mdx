...
// Extract thinking from message parts
...
- The reasoning stream is emitted through `ThinkingPart` in message parts
- Access thinking via: `message.parts.whereType<ThinkingPart>()`
...
await for (final chunk in agent.sendStream(
  ...
  // Extract thinking from message parts
  final thinking = chunk.messages
      .expand((m) => m.parts)
      .whereType<ThinkingPart>()
      .map((p) => p.text)
      .join();
  final hasThinking = thinking.isNotEmpty;
...
The stream delivers reasoning deltas incrementally through `ThinkingPart` in
message parts, so you can render a live "thought bubble" while the model is
working. Each chunk may include text output, thinking, or both.
