...
Hand-writing schemas and JSON serialization is a lot of boilerplate. Use
`json_serializable` with `createJsonSchema: true` to automate this:
...
// now we pass the schema and JSON serialization to the agent
