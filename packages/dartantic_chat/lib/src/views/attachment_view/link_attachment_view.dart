...
/// - Page title (extracted from URL or provided in the part)
...
class LinkAttachmentView extends StatelessWidget {
  /// Creates a [LinkAttachmentView] for the given [linkPart].
  ///
  /// The [linkPart] must not be null and should contain at least a valid URL.
  /// If [linkPart.name] is provided, it will be used as the display title.
  LinkAttachmentView(this.linkPart, {super.key}) : _contextKey = GlobalKey();

  /// The link part to display.
  ...
  ///
  /// Uses the part name if provided and not the same as the URL.
  /// Otherwise, extracts a title from the URL.
  String _getPageTitle(LinkPart linkPart) {
    final name = linkPart.name;
    if (name != null &&
        name.isNotEmpty &&
        name != linkPart.url.toString() &&
        !name.startsWith('http')) {
      return name;
    }
    return _extractTitleFromUrl(linkPart.url);
  }
}
