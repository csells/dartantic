...
>
> Every message mapper that converts `List<ChatMessage>` to provider-specific format MUST include
> an assertion at the entry point that fails if any `ThinkingPart` appears in outbound messages:
>
> ```dart
...
> ```
>
> This assertion exists in ALL provider message mappers (Anthropic, Google, OpenAI, OpenAI Responses,
> Mistral, Ollama). When implementing a new provider, include this assertion in your message mapper.
